...
void FlashDeviceManager::NotifyDiscoverComplete(esp_gatt_if_t gattc_if,uint16_t conn_id)
{
	uint8_t i = 0;
	for(i = 0; i < this->numberOfDevices; i++)
	{
		if(this->listOfDevice[i].conn_id == conn_id)
		{
			this->listOfDevice[i].flashService.NotifyDiscoverComplete(gattc_if,conn_id);
			break;
		}
	}
}
void FlashDeviceManager::RefreshServiceDiscover(uint16_t conn_id)
{
	...
	for(i = 0; i < this->numberOfDevices; i++)
	{
		if(this->listOfDevice[i].conn_id == conn_id)
		{
			this->listOfDevice[i].flashService.RefreshServiceInfo(this->listOfDevice[i].bda);
			break;
		}
	}
}
void FlashDeviceManager::RequestVersion(esp_gatt_if_t gattc_if, uint16_t conn_id)
{
	uint8_t i = 0;
	for(i = 0; i < this->numberOfDevices; i++)
	{
		if(this->listOfDevice[i].conn_id == conn_id)
		{
			this->listOfDevice[i].flashService.RequestVersion(gattc_if,conn_id);
			break;
		}
	}
}

void FlashDeviceManager::NotifyEvtChr(uint16_t conn_id,uint16_t handle,uint8_t *value,uint16_t value_len,uint8_t type)
{
	uint8_t i = 0;
	for(i = 0; i < this->numberOfDevices; i++)
	{
		if(this->listOfDevice[i].conn_id == conn_id)
		{
			this->listOfDevice[i].flashService.NotifyChrEvent(handle,value,value_len,type);
			break;
		}
	}
}
void FlashDeviceManager::RegisterForNotifyBlockReq(esp_gatt_if_t gattc_if, uint16_t conn_id)
{
	uint8_t i = 0;
	for(i = 0; i < this->numberOfDevices; i++)
	{
		if(this->listOfDevice[i].conn_id == conn_id)
		{
			this->listOfDevice[i].flashService.RegisterReqBlockForNotification(gattc_if,this->listOfDevice[i].bda);
		}
	}
}
