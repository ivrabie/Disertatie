...
uint8_t OLED::draw_string(uint8_t x, uint8_t y, etl::string<SSD1306_MAX_SIZE> str,
		ssd1306_color_t foreground, ssd1306_color_t background) {
	uint8_t t = x;
	...
	for (int i = 0; i < str.size(); i++) {
		x += draw_char(x, y, str[i], foreground, background);
		if (str[i] != 0 && str[i] != ' ')
			x += font->c;
	}
...
uint8_t OLED::measure_string(etl::string<SSD1306_MAX_SIZE> str) {
	uint8_t w = 0;
	unsigned char c;
...
void OLED::clean_position(uint8_t x, uint8_t lines)
{
	if(type == SSD1306_128x64)
	{
		uint16_t start_idx = ((uint16_t)x*128)/8u;
		uint16_t len = ((uint16_t)lines*128)/8u;
		memset(&buffer[start_idx],0,len); 
		if(start_idx < 1024)
		{
			if((start_idx+len) >= 1024)
			{
				len = 1024 - start_idx;
			}
			// memset(&buffer[start_idx],0,len); 
	...
	{
		uint16_t start_idx = ((uint16_t)x*128);
		uint16_t len = (uint16_t)(lines*128)/8u;
		if(start_idx < 512)
		{
			if((start_idx+len) >= 512)
			{
				len = 512 - start_idx;
			}
			memset(&buffer[start_idx],0,len); 
